...
private:
    ...
    std::vector<uint8_t> handshake1(const std::vector<uint8_t>& local_seed,
        ...
        if (response_code != 200) {
            Serial.print(F("TAPO_PROTOCOL: Handshake1 failed, code="));
            Serial.println(String(response_code));
            return {};
        }

        // Must be >= 16(remote seed) + 32(hash) bytes to avoid out-of-range access.
        if (response_str.length() < 48) {
            Serial.print(F("TAPO_PROTOCOL: Handshake1 response too short: "));
            Serial.println(String(response_str.length()));
            return {};
        }
        ...
        if (local_hash != server_hash) {
            Serial.println(F("TAPO_PROTOCOL: Invalid credentials during handshake1"));
            return {};
        }
        TAPO_PROTOCOL_DEBUG(F("Handshake1 successful"));
    ...
    bool handshake2(const std::vector<uint8_t>& local_seed,
        ...
        if (response_code != 200) {
            Serial.print(F("TAPO_PROTOCOL: Handshake2 failed with response code "));
            Serial.println(String(response_code));
            return false;
        }
        TAPO_PROTOCOL_DEBUG(F("Handshake2 successful"));
        return true;
    }
};
